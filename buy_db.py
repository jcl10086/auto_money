...
def trade_data(results):
    ...
    df = df[df['current_price'].astype(float) >= (df['zt_price'].astype(float) * 0.982).round(2)]
...
def get_codes():
    global codes
    df = pywencai.get(query='沪深主板非st，昨日未涨停，前日未涨停，当前涨幅>2.5且<8，流值小于60亿且大于30亿，股价<9且>3，无可转债，今日最大涨幅<11', loop=True, sort_order='desc', sort_key='最新涨跌幅', pro=True, cookie=cookie)
