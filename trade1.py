...
def get_codes():
    df = pywencai.get(query='7月10日连板，7月11日未涨停，沪深主板非st', loop=True, sort_order='desc', sort_key='最新涨跌幅', pro=True, cookie=cookie)
...
def get_data(stock_list):
    ...
    # 当前涨跌幅
    my_df['pl_last_zf'] = (my_df['price'] - my_df['last_close']) / my_df['last_close'] * 100
    # 最小跌幅
    my_df['ll_zf'] = (my_df['low'] - my_df['last_close']) / my_df['last_close'] * 100
    # 开盘价 - 最低价
