...
def trade_data(results):
    ...
    df = merged[['code', 'current_price', 'current_count', 'zt_price']]
    ...
    df = df[df['current_count'].astype(float) >= 1000000]
...
def get_codes():
    global codes
    df = pywencai.get(query='沪深主板非st，昨日未涨停，前日未涨停，当前涨幅>2.5且<8，流值小于60亿，股价<18且>3，无可转债，今日最大涨幅<9.8且>6', loop=True, sort_order='desc', sort_key='最新涨跌幅', pro=True, cookie=cookie)
