...
def trade_data(results):
    ...
    df = merged[['code', 'current_price', 'zt_price', 'last_close']]
    current_time = datetime.now()
    ts = current_time.strftime('%H:%M:%S.%f')[:-3]
    df['ts'] = ts
    print(df)
    # df = df[(df['buy1_price'] == df['zt_price']) & (df['buy1_quantity'] > 5000000)]
    df = df[df['current_price'].astype(float) >= (df['last_close'].astype(float) * 1.05).round(2)]
...
def get_codes():
    global codes
    df = pywencai.get(query='沪深主板非st，昨日未涨停，涨幅>2且<5，今日最大涨幅<5，开盘涨跌幅>-2，股价>3且<30，流值<150亿', loop=True, sort_order='desc', sort_key='最新涨跌幅', pro=True, cookie=cookie)
