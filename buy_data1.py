...
def get_codes():
    df = pywencai.get(query='开盘涨幅>=2，沪深主板非st，卖一>0，昨日未涨停，股价>2', loop=True, sort_order='desc', sort_key='最新涨跌幅')
...
def get_data(stock_list):
    ...
    my_df = my_df[(my_df['reversed_bytes9'] >= 2) & (my_df['price'] == my_df['zt_price']) & (my_df['ask_vol1'] < 8000)]
