...
def get_codes():
    df = pywencai.get(query='开盘涨跌幅>=0且<=3，流值小于50亿，创业板，前5日涨跌幅>7的次数=0', loop=True, sort_order='desc', sort_key='最新涨跌幅')
...
def get_data(stock_list):
    ...
    my_df = my_df[(my_df['reversed_bytes9'] >= 4) & (my_df['reversed_bytes9'] <= 6)]
